...
/**
 * Helper class for testing checks without having to deploy them on a Sonar
 * instance. It can be used as following:
 * <pre>{@code
 * CheckMessagesVerifier.verify(messages)
 *   .next().atLine(1).withMessage("foo")
 *   .next().atLine(2).withMessage("bar")
 *   .noMore();
 * }</pre> Strictly speaking this is just a wrapper over collection of
 * {@link HtmlIssue} which guarantees order of traversal.
 *
